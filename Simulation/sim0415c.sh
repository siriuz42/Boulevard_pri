...
node.work <- function(dummy, n, d, subsample, leaf.size, ntree, error, lambda = 0.5) {
...
result <- sfSapply(1:1000, node.work, 
                   n=n, d=d, subsample=subsample, error=error4, leaf.size=leaf.size, ntree=ntree)
result <- t(result)
save(result, file=paste("result_c_", suffix, ".RData", sep=""))
